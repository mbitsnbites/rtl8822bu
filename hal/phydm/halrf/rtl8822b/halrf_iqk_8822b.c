...
)
{
	...
	for (path = 0; path < 2; path++) {
		...
		for (idx = 0; idx < reload_idx; idx++) {
			...
			if (idx == 0)
				odm_set_bb_reg(p_dm_odm, iqk_apply[path], BIT(0), (p_iqk_info->IQK_fail_report[channel][path][idx] ? 0 : 1));
			else
				odm_set_bb_reg(p_dm_odm, iqk_apply[path], BIT(10), (p_iqk_info->IQK_fail_report[channel][path][idx] ? 0 : 1));
		}
		odm_set_bb_reg(p_dm_odm, 0x1bd8, MASKDWORD, 0x0);
		odm_set_bb_reg(p_dm_odm, 0x1b0c, BIT(13) | BIT(12), 0x0);
	}
